...
contract GasBenchmarks is Test {
    ...
    function test_GasBenchmark_ProposalMetadataStorageCost() public {
        ...
        for (uint256 i = 0; i < 4; i++) {
            uint256 gasBefore = gasleft();
            vm.prank(validators[i]);
            proposalManager.createProposal(keccak256(abi.encodePacked("test", i)), metadataSizes[i]);
    ...
    function test_GasBenchmark_DisputeResolutionScaling() public {
        ...
        for (uint256 i = 2; i < 18; i++) {
            ...
            disputeResolver.voteOnDispute(
                disputeId, supportChallenge, createDisputeVoteSignature(privateKey, disputeId, supportChallenge)
            );
    ...
    function test_GasBenchmark_VotingPatternComparison() public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            vm.prank(validators[i]);
            proposalIds[i] = proposalManager.createProposal(keccak256(abi.encodePacked("pattern", i)), "Pattern Test");
        ...
        for (uint256 i = 0; i < 10; i++) {
            uint256 gasBefore = gasleft();
            vm.prank(validators[i]);
            consensusEngine.castVote(roundIds[0], true, createVoteSignature(validatorPrivateKeys[i], roundIds[0], true));
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            consensusEngine.castVote(
                roundIds[1], i % 2 == 0, createVoteSignature(validatorPrivateKeys[i], roundIds[1], i % 2 == 0)
            );
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            uint256 gasBefore = gasleft();
            vm.prank(validators[i]);
            consensusEngine.castVote(roundIds[2], vote, createVoteSignature(validatorPrivateKeys[i], roundIds[2], vote));
